...
function Exp14Day() {
    ...
    useEffect(() => {
        const fetchExpiringItems = async () => {
            try {
                const response = await axios.get('http://localhost:8081/api/items/expiring14');
    ...
    return (
        <div>
            <div className="exp-container">
                <h1 className="exp-title">Expiry 14 Day</h1>
                ...
                <div className="exp-items-table-container">
                    {expiringItems.length > 0 ? (
                        <table className="exp-items-table">
                            ...
                            <tbody>
                                {expiringItems.map((item) => {
                                    ...
                                    if (diffDays < 0) status = `(Expired ${Math.abs(diffDays)} Days)`;
                                    else if (diffDays === 0) status = '(Today)';
                                    else if (diffDays === 1) status = '(Tomorrow)';
                                    else if (diffDays <= 7) status = `(${diffDays} Days)`;
                                    else if (diffDays <= 14) status = `(${diffDays} Days)`;
                                    ...
                                    return (
                                        <tr key={item._id}>
                                            ...
                                            <td>{item.quantity}</td>
                                            <td>
                                                {expiryDate.toLocaleDateString('en-GB')} {status}
                                            </td>
                        ...
                        <p className="exp-no-items">No items expiring within 14 days.</p>
                    )}
                </div>
            </div>
        </div>
